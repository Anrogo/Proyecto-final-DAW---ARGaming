<!-- Page Content -->
<div class="container mt-3">

    <div class="row">

        <!-- Post Content Column -->
        <div class="col-12 col-lg-8">

            <ul class="breadcrumb">
                <li class="breadcrumb-item small"><a href="/">Home</a></li>
                <li class="breadcrumb-item small"><a href="/post/novedades">Novedades</a></li>
                <li class="breadcrumb-item small"><a href="#">2020</a></li>
                <li class="breadcrumb-item small"><a href="/post/novedades/3">Call of Duyt: Modern Warfare y Warzone</a></li>
            </ul>

            <!-- Titulo -->
            <h2 class="mt-4 text-primary">La temporada 4 de ‘Call of Duty: Modern Warfare’ y ‘Warzone’ ya está aquí: estas son sus novedades</h2>

            <!-- Descripción -->
            <h2 class="small text-justify">La temporada 4 de de 'Call of Duty: Modern Warfare' y 'Warzone' ya se encuentra entre nosotros tras el retraso. Repasamos sus novedades.</h2>

            <!-- Autor -->
            <p class="lead">
                By
                <a href="/usuario/1">Anrogo</a>
            </p>

            <hr>

            <!-- Imagen del post -->
            <img class="img-fluid rounded" src="/images/post/modern-warfare-1.webp" alt="post">

            <!-- Fecha / hora -->
            <p class="small">Posteado el 13/06/2020 a las 20:12:12</p>

            <hr>

            <!-- Contenido del post -->

            <p class="text-justify">
                Tras unos días de retraso debido a las protestas contra el racismo en Estados Unidos, Activision e Infinity Ward han lanzado la temporada 4 de Call of Duty: Modern Warfare y Call of Duty: Warzone. La puedes descargar desde ahora en PlayStation 4, Xbox One y PC. Desde luego, la cuarta etapa llega cargada de novedades tanto en el multijugador tradicional como en el battle royale. Las repasamos todas a continuación.
            </p>
            <p class="text-justify">
                En primer lugar hablaremos de Call of Duty: Modern Warfare. Su multiplayer da la bienvenida al mapa Scrapyard, uno de los más emblemáticos de Modern Warfare 2. No obstante, el estudio promete que en las próximas semanas agregará otros escenarios, entre ellos Cheshire Park. Por su parte, el modo Gunfight igualmente recibe un nuevo mapa: Trench. A lo largo de la temporada podrás disfrutar de nuevas modalidades como Team Defender, One in the Chamber y All or Nothing.
            </p>

            <hr>

            <iframe width="740" height="441" src="https://www.youtube.com/embed/7N8VDutRs_U" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

            <hr>

            <p class="text-justify">
                Respecto a los elementos que compartirán Call of Duty: Modern Warfare y Warzone, encontramos al operador protagonista de la temporada 4, el Capitán Price. Más adelante se abrirán las puertas a Gaz y Rose. El arsenal se verá ampliado con la incorporación de las Fennec (SMG) y CR-56 AMAX (AR). Después podrás equiparte com las Rytec AMR (Sniper) y Kali Sticks.
            </p>
            <p class="text-justify">
                ¿Cuáles son las novedades de Warzone? El battle royale tendrá una variante: Juggernaut Royale. Consiste en un modo en el que tres paquetes dejarán el traje juggernaut en Verdansk. Los jugadores que logren obtenerlos tendrán salud más alta una mini pistola e incluso se activará una canción especial de fondo. Cabe señalar que, pese al peso del atuendo, los usuarios podrán subir a vehículos o escaleras. Asimismo, si caes desde una superficie alta no solo recibirás un daño mínimo, también perjudicarás a los contrincantes que estén cerca de la zona de impacto.
            </p>

            <img class="img-fluid rounded" src="/images/post/modern-warfare-2.webp" alt="foto-post">

            <p class="text-justify">
                Warzone también tendrá una modalidad realista en la que el HUD solo mostrará elementos esenciales. Además, se incrementa significativamente el daño a la cabeza. Recuerda que la cuarta temporada viene acompaña de un nuevo Season Pass, el cual te ofrecerá desbloquear todo tipo de objetos cosméticos como skins para armas y operadores, relojes, llaveros y puntos CP. En el siguiente tráiler puedes ver gran parte de los objetos disponibles en el pase de temporada de Call of Duty: Modern Warfare y Warzone:
            </p>

            <iframe width="740" height="441" src="https://www.youtube.com/embed/esbKS_7yqpw" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

        </div>
        <!-- Sidebar Widgets Column -->
        <div class="col-12 col-lg-4">
            <!-- Categories Widget -->
            <div class="card my-4">
                <h5 class="card-header">Post de usuarios</h5>
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6">
                            <ul class="list-unstyled mb-0">
                                <li>
                                    <a href="/post/2">¿Cómo unirse?</a>
                                </li>
                                <li>
                                    <a href="/post/11">Final Fantasy VII: Remake</a>
                                </li>
                            </ul>
                        </div>
                        <div class="col-lg-6">
                            <ul class="list-unstyled mb-0">
                                <li>
                                    <a href="/post/1">Bienvenidos al blog!</a>
                                </li>
                                <li>
                                    <a href="/post/8">Mi post con foto</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Side Widget -->
            <div class="card my-4">
                <h5 class="card-header">GTA V, no hay que decir más</h5>
                <div class="card-body">
                    <iframe class="iframe-video" src="https://www.youtube.com/embed/hBvMSP7cI-Q" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
            </div>

            <!-- Links relacionados -->
            <div class="card my-4">
                <h5 class="card-header">Otros enlaces que te pueden interesar</h5>
                <div class="card-body">
                    <ul class="list-group">
                        <li class="list-group-item"><a href="https://www.eurogamer.es/articles/2020-06-16-gta-5-juego-mas-vendido-mayo-en-espana" target="_blank">GTA V fue el videojuego más vendido durante el mes de mayo en España</a></li>
                        <li class="list-group-item"><a href="https://latam.historyplay.tv/hoy-en-la-historia/se-lanza-tennis-two-considerado-el-primer-videojuego-de-la-historia" target="_blank">Primer juego de la historia</a></li>
                        <li class="list-group-item"><a href="https://qpasa.com/actualidad/los-10-video-juegos-mas-populares-de-este-2020/" target="_blank">Los 10 videojuegos más exitosos del momento</a></li>
                        <li class="list-group-item"><a href="https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&cad=rja&uact=8&ved=2ahUKEwjkzvfhjvDpAhV0AWMBHWBNCf8QFjACegQIBBAB&url=https%3A%2F%2Fwww.20minutos.es%2Fvideojuegos%2Fnoticia%2F4059964%2F0%2Fgoogle-play-store-juego-mas-descargado-historia-subway-surfers%2F&usg=AOvVaw1yKoPGKJaOWVaIK09oBU8j" target="_blank">El juego con más descargas del 2019</a></li>
                        <li class="list-group-item"><a href="https://vandal.elespanol.com/noticia/1350734838/square-enix-esta-satisfecha-con-las-ventas-de-final-fantasy-vii-remake/" target="_blank">Square Enix está satisfecha con las ventas de Final Fantasy VII: Remake</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>